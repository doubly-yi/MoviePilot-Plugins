...
<template>
  <div class="bp-manager">
    <div class="bp-header">
      ...
      <div class="bp-header-actions"><VBtn v-if="!isSettings" variant="text" size="small" :loading="loading" @click="refresh">刷新</VBtn><VBtn variant="text" size="small" @click="emit('close')">关闭</VBtn></div>
    </div>
...
.bp-header{margin-bottom:28px}.bp-header-actions{display:flex}.bp-manager h2{font-size:18px;font-weight:600}.bp-manager h3{font-size:15px;font-weight:600}.bp-hint,.bp-phone{font-size:12px;opacity:.65;margin:4px 0 0}
...
.bp-config-options{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px}.bp-accounts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.bp-account{border-color:rgba(var(--v-theme-on-surface),.12);border-radius:12px}.bp-account-actions{gap:6px;margin-top:16px}.bp-empty{text-align:center;border:1px dashed rgba(var(--v-theme-on-surface),.2);border-radius:12px;padding:36px 16px}.bp-empty p{opacity:.6;margin:8px 0 20px}.bp-dialog{padding:12px;border-radius:16px}.bp-phone{font-variant-numeric:tabular-nums}.bp-config-footer{display:flex;justify-content:space-between;position:sticky;bottom:0;background:rgb(var(--v-theme-surface));border-top:1px solid rgba(var(--v-theme-on-surface),.1);margin-top:28px;padding:16px 0 0}
