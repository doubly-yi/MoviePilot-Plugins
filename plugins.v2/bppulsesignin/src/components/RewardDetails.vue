...
const props = defineProps({ account: Object, request: Function })
const emit = defineEmits(['updated'])
const open = ref(false)
const loading = ref(false)
const error = ref('')
const pending = r => ['待领取', '可领取'].includes(r.status_text.trim())
const pendingCount = computed(() => (props.account.rewards || []).filter(pending).length)
const color = r => pending(r) ? 'warning' : r.status_text === '已发放' ? 'success' : 'secondary'
...
function show() { open.value = true; refresh() }
</script>

<template>
  <VBtn size="small" variant="text" :color="account.reward_error ? 'warning' : pendingCount ? 'warning' : 'primary'" @click="show">{{ account.reward_error ? '奖励查询失败' : pendingCount ? `待领取 ${pendingCount} 项` : account.reward_updated ? '奖励详情' : '奖励待同步' }}</VBtn>
  <VDialog v-model="open" max-width="620">
    <VCard class="bp-reward-dialog"><VCardTitle>签到奖励 · {{ account.name }}</VCardTitle><VCardText>
      <div class="bp-reward-header"><span>更新：{{ when }}</span><VBtn size="small" variant="text" color="primary" :loading="loading" @click="refresh">刷新奖励</VBtn></div>
    ...
    </VCardText><VCardActions><VSpacer/><VBtn @click="open = false">关闭</VBtn></VCardActions></VCard>
  </VDialog>
</template>
